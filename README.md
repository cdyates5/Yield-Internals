...
addresses (`HTTP 429`). Locally the dashboard uses Yahoo and needs no key; on GitHub it uses **Tiingo** for the ETFs
(a free tier with 30-plus years of history) and **FRED** for the yen crosses and the dollar. FRED needs no key.
...
| USD/JPY, AUD/JPY | FRED daily reference rates | Tiingo Forex only goes back to 2020; the model needs 2004. USD/JPY = DEXJPUS; AUD/JPY = DEXUSAL × DEXJPUS. One business-day lag; weekly-change correlation to the market close 0.86 and 0.91 |
...
|---|---|---|
| `TIINGO_TOKEN` missing or wrong on CI | The build fails in the first few seconds with a message naming the fix, rather than hanging | **No** |
...
- **FRED**: the two yen crosses (USD/JPY from DEXJPUS, AUD/JPY from DEXUSAL × DEXJPUS), the broad trade-weighted dollar
  (the context-only dollar internal), and the yield fallback if Treasury.gov is unreachable. FRED FX lags one business day, which is fine for a weekly model.
