...
|---|---|---|
| A Yahoo ticker fails to download (rate limit, outage) | Uses the cached copy from the previous run; an orange banner lists the stale tickers | Yes |
...
- **Yahoo Finance**: daily total-return adjusted closes for sector and style ETFs, USD/JPY and AUD/JPY, the DXY index, and COMEX/NYMEX
  continuous front-month futures. FX and futures are dated to the exchange-local session.
