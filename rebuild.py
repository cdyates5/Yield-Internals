        ...
        for attempt in range(2):                          # a second pass re-handshakes if the crumb went stale
            for host in ("query1", "query2"):
                ...
                try:
                    ...
                    if len(ser) < 250: raise RuntimeError(f"{sym}: only {len(ser)} rows")
                    ser.to_csv(path, header=["v"])
                    err = None
                    break
...
def main():
    ...
    D = {}
    for t in TICKERS:
        D[t] = yahoo(t); time.sleep(0.5 + 1.5 * random.random())   # gentle + jittered from shared CI IP ranges
    D = pd.DataFrame(D)
    y10, real, bei = yields()
