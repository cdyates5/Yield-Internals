...
def _fred_api(fid, tries=5, base_wait=2, max_wait=30):
    """FRED official JSON API (api.stlouisfed.org). Reliable from CI, unlike the fredgraph CSV chart endpoint
    (fred.stlouisfed.org/graph), which resets connections from GitHub's runner IPs. Needs a free FRED_API_KEY."""
    ...
    txt = http_get(url, headers=["Accept: application/json"], tries=tries, base_wait=base_wait, max_wait=max_wait)
    j = json.loads(txt)
    if "observations" not in j:
        raise RuntimeError(f"FRED API {fid}: {txt[:120]!r}")
    obs = j["observations"]
    s = pd.Series({o["date"]: o["value"] for o in obs})
    s.index = pd.to_datetime(s.index)
    return pd.to_numeric(s, errors="coerce").dropna()   # FRED encodes missing days as "."

def _fredgraph(fid, tries=3, base_wait=2, max_wait=15):
    """FRED series, JSON API first (works from CI), fredgraph CSV as a local fallback. Cached to disk as CSV.
    Bounded retry budget: callers of non-critical series keep this short and fall back to cache."""
    path = os.path.join(CACHE, f"f_{fid}.csv")
    if not fresh(path):
        s = None
        if FRED_API_KEY:
            try:
                s = _fred_api(fid, tries=tries + 1)
            except Exception as e:
                log(f"  FRED API {fid} failed ({str(e)[:60]}); trying fredgraph")
        if s is None:                                    # no key, or API failed: fredgraph CSV (works locally)
            ...
            s = pd.Series(pd.to_numeric(df.iloc[:, 1], errors="coerce").values, index=pd.to_datetime(df.iloc[:, 0])).dropna()
        s.rename_axis("observation_date").rename(fid).to_csv(path)
    df = pd.read_csv(path)
...
def fred_check():
    """Quick check that the FRED JSON API key works (the yen crosses depend on it on CI)."""
    try:
        s = _fred_api("DEXJPUS", tries=3, base_wait=2, max_wait=15)
        return (len(s) > 100), ("ok" if len(s) > 100 else "too few observations")
    except Exception as e:
        return False, str(e)[:160]
...
def main():
    ...
    # On CI the yen crosses come from FRED's JSON API (fredgraph CSV resets from GitHub IPs), so a key is required there.
    if on_ci and not FRED_API_KEY:
        raise RuntimeError("No FRED_API_KEY on CI. The yen crosses need FRED's JSON API (the fredgraph CSV endpoint "
                           "resets from GitHub runner IPs). Add the free FRED_API_KEY secret; see the README.")
    if FRED_API_KEY:
        fok, fwhy = fred_check()
        if not fok:
            if on_ci:
                raise RuntimeError(f"FRED_API_KEY is set but not working ({fwhy}). Get a free key at "
                                   "https://fred.stlouisfed.org/docs/api/api_key.html and update the FRED_API_KEY secret.")
            log(f"WARNING: FRED_API_KEY not working ({fwhy}); using fredgraph CSV instead.")
        else:
            log("FRED API key OK.")
