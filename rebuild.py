...
Built to run unattended (GitHub Actions) as well as locally.
  ...
  python rebuild.py --out site --cache cache --max-age-days 5
...
def _yahoo_session(force=False):
    ...
    for i in range(5):
        ...
        if code == "200" and crumb and "<" not in crumb and len(crumb) < 40:
            break
        time.sleep(min(60, 4 * (2 ** i)) * (0.6 + 0.8 * random.random()))
    _YSESSION.update(cookie=ck if os.path.exists(ck) else None, crumb=crumb or None)
    return _YSESSION

def _yahoo_raw(sym):
    """Fetch a full daily series from Yahoo and return it (no disk cache; series() owns caching)."""
    sess = _yahoo_session()
    esym = urllib_quote(sym)
    err = None
    for attempt in range(2):                              # a second pass re-handshakes if the crumb went stale
        for host in ("query1", "query2"):
            ...
            try:
                j = json.loads(http_get(url, headers=BROWSER_HEADERS, cookie=sess.get("cookie")))["chart"]["result"][0]
                q = j["indicators"]
...
TIINGO_FX = {"JPY=X":"usdjpy", "AUDJPY=X":"audjpy"}
...
SRC = {}     # ticker -> source actually used ("Tiingo", "Tiingo FX", "FRED", "Yahoo")
...
def tiingo_daily(sym, tsym):
    ...
    txt = http_get(url, headers=["Accept: text/csv"])
...
def tiingo_fx(sym, tsym):
    url = (f"https://api.tiingo.com/tiingo/fx/{tsym}/prices?startDate=2000-01-01&resampleFreq=1day"
           f"&format=csv&columns=date,close&token={TIINGO_TOKEN}")
    txt = http_get(url, headers=["Accept: text/csv"])
    if not txt[:4].lower().startswith("date"):
        raise RuntimeError(f"Tiingo FX {tsym}: {txt[:100]!r}")
    df = pd.read_csv(io.StringIO(txt))
    s = pd.Series(pd.to_numeric(df["close"], errors="coerce").values, index=pd.to_datetime(df["date"])).dropna()
    s.index = s.index.tz_localize(None) if s.index.tz is not None else s.index
    return s   # USD per unit: usdjpy = JPY per USD, matches Yahoo JPY=X (higher = weaker yen = higher yields prior)
...
def fetch_series(sym):
    ...
    if not fresh(path):
        errs = []
        # ordered list of (source_label, callable) to try
        plan = []
        if TIINGO_TOKEN and sym in TIINGO_ETF: plan.append(("Tiingo", lambda: tiingo_daily(sym, TIINGO_ETF[sym])))
        if TIINGO_TOKEN and sym in TIINGO_FX:  plan.append(("Tiingo FX", lambda: tiingo_fx(sym, TIINGO_FX[sym])))
        if sym in FRED_PROXY:                  plan.append(("FRED", lambda: _fredgraph(FRED_PROXY[sym])))
        plan.append(("Yahoo", lambda: _yahoo_raw(sym)))   # always the last resort; works off-CI
        got = None
        for label, fn in plan:
            try:
                s = fn()
                s = s[~s.index.duplicated(keep="last")].dropna()
                if len(s) < 250: raise RuntimeError(f"{sym}: only {len(s)} rows from {label}")
                s.to_csv(path, header=["v"]); SRC[sym] = label; got = label; break
            except Exception as e:
        ...
        if got is None:
            ...
            log(f"  {sym} all sources failed; using cached copy to {last.date()} ({errs[-1]})")
    else:
...
def main():
    ...
    meta = {c[0]: dict(key=c[0], label=c[1], legs=c[2], origin=c[3], sign=c[4], pillar=c[5]) for c in CANDS}
...
(function legsNote(){
  ...
  $("legsNote").textContent = `Most internals price the breakeven leg. USD/JPY is the main real-yield member (${fR(uj.rreal)} real, ${fR(uj.rbei)} breakeven). The dollar index loads ${fR(dxy.rreal)} on real yields but ${fR(dxy.rbei)} on breakevens, so on the nominal yield the two legs cancel; that is why it fails the screen despite a genuine rate-differential link. TIPS / Treasuries at ${fR(tip.rbei)} is close to the breakeven itself.`;
})();
function cmat(){
...
(function method(){
  const ys = M.ysrc || {};
  const usedTiingo = Object.values(M.src || {}).includes("Tiingo") || Object.values(M.src || {}).includes("Tiingo FX");
  ...
  const srcLine = usedTiingo
    ? `Equity and style ETFs and the two yen crosses come from Tiingo; ${activeProxies.length ? activeProxies.join(", ") + " are ETF or index proxies (the model is built on log-change ratios, where a liquid ETF tracks the front-month future at a weekly-change correlation of 0.92 to 0.98)" : "commodities use their liquid ETFs"}. Prices are dated to the exchange-local session.`
...
def cli():
    ...
    ap.add_argument("--max-age-days", type=int, default=5, help="exit 2 if the as-of date is older than this (calendar days, US/Eastern)")
