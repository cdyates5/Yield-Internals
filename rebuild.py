...
# ---------------------------------------------------------------- fetch
def http_get(url, binary=False, tries=5):
    """curl with retry/backoff on 429, 5xx and network errors. Returns bytes or str; raises after `tries`."""
    last = ""
    for i in range(tries):
        with tempfile.NamedTemporaryFile(delete=False) as tf: tmp = tf.name
        try:
            r = subprocess.run(["curl", "-s", "-L", "--compressed", "--max-time", "60", "-H", f"User-Agent: {UA}",
...
def yahoo(sym):
    path = os.path.join(CACHE, "y_" + re.sub(r"[^A-Za-z0-9]", "_", sym) + ".csv")
    if not fresh(path):
        err = None
        for host in ("query1", "query2"):
            url = (f"https://{host}.finance.yahoo.com/v8/finance/chart/{sym}?period1=946684800&period2={int(time.time())}"
                   "&interval=1d&events=div%2Csplit&includeAdjustedClose=true")
            try:
                j = json.loads(http_get(url, tries=4))["chart"]["result"][0]
                q = j["indicators"]
                ...
                # (Futures and FX are skipped: their evening sessions have already rolled into the next trade date.)
                ...
                s = pd.Series(vals, index=ix, dtype=float)
                s = s[~s.index.duplicated(keep="last")].dropna()
                if len(s) < 250: raise RuntimeError(f"{sym}: only {len(s)} rows")
                s.to_csv(path, header=["v"])
                err = None
                break
        ...
        if err is not None:
            if not os.path.exists(path):
                raise RuntimeError(f"Yahoo {sym} failed and no cache: {err}")
            last = pd.read_csv(path, index_col=0, parse_dates=True).index.max()
            STALE[sym] = str(last.date())
            log(f"  Yahoo {sym} failed ({err}); using cached copy to {last.date()}")
...
def main():
    ...
    D = {}
    for t in TICKERS:
        D[t] = yahoo(t); time.sleep(0.3)             # gentle on Yahoo from shared CI IP ranges
    D = pd.DataFrame(D)
    y10, real, bei = yields()
    ...
    payload = dict(meta=dict(built=dt.datetime.now(dt.timezone.utc).strftime("%Y-%m-%d %H:%M UTC"), asof=str(asof.date()),
                             ...
                             pillar_names=PILLAR_NAMES, ysrc=YSRC, stale=STALE, run_url=run_url(), members=MEMBERS, regimes=[r[2] for r in regimes], dur=dur,
...
(function method(){
  ...
  const P = [
    ...
    `<b>Internals.</b> Weekly log change of each ratio. Baskets are equal-weight averages of member ETF log changes. ETF prices are Yahoo Finance total-return adjusted closes, so dividend gaps between utilities and the market do not leak into the ratios. FX and futures are dated to the exchange-local session.`,
