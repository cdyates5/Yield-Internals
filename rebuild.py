...
def fetch_series(sym):
    ...
    if not fresh(path):
        ...
        if TIINGO_OK and sym in TIINGO_FX:  plan.append(("Tiingo FX", lambda: tiingo_fx(sym, TIINGO_FX[sym])))
        ...
        # fast mode (short budget) and, when the symbol already has a Tiingo mapping, is skipped entirely (blocked IPs).
        if not (TIINGO_OK and (sym in TIINGO_ETF or sym in TIINGO_FX)):
...
def main():
    ...
    TIINGO_OK = ok
    if ok:
        log("Tiingo token OK: fetching ETFs and FX from Tiingo, dollar from FRED, yields from Treasury.gov")
    elif TIINGO_TOKEN:
